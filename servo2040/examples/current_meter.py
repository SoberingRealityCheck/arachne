...
while not user_sw.raw():
    if iter > 90:
        iter = 0.0
    iter += 1
    for servonum in range(7):
        print("Servo ", servonum + 1, ":", end=" ")
        servoval = math.sin(math.radians(iter * 4 + servonum * 20)) * 0.5 + 0.5
        print("Setting servo", servonum, "to", round(servoval, 2))
        # Set the servo value to a sine wave, so we can see the current change
        # This is just for demonstration purposes, you can set it to any value
        time.sleep(.5)
        servos.value(servonum, servoval)
        print("success.")

    print("currently:", servos.value(1))
